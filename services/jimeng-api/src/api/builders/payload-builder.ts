...
/**
 * benefitCount 规则
 * - 生图模式默认返回 1（节省积分），可通过 count 参数覆盖
 * - 多图模式: 不加
 */
...
export function buildCoreParam(options: BuildCoreParamOptions) {
  ...
  const effectiveIntelligentRatio = ['jimeng-4.0', 'jimeng-4.1', 'jimeng-4.5', 'jimeng-4.6', 'jimeng-5.0'].includes(userModel)
    ? (intelligentRatio === undefined ? true : intelligentRatio)
    : false;
...
export function buildMetricsExtra({
  ...
  isMultiImage = false,
  count = 1,
  omitSceneOptions = false,
}: BuildMetricsExtraOptions): string {
  const benefitCount = getBenefitCount(userModel, regionInfo, isMultiImage, count);
  ...
  const metrics: any = {
    promptSource: "custom",
    generateCount: benefitCount !== undefined ? benefitCount : 1,
    enterFrom: "click",
    generateId: submitId,
    isRegenerate: false,
  };
  if (!omitSceneOptions) {
    metrics.sceneOptions = JSON.stringify([sceneOption]);
  } else {
    // 对齐即梦 web 端按张计费（PerPiece）请求：不带 sceneOptions
    metrics.position = "page_bottom_box";
    metrics.isBoxSelect = false;
    metrics.isCutout = false;
    metrics.hasRejectedAudit = 0;
  }
...
export function buildDraftContent({
  ...
  if (generateType === "generate") {
    ...
    // 即梦 web 端「生成数量」：gen_option 与 generate 平级（web 抓包确认），
    // 服务端缺省一次生成 4 张，必须显式传 gen_count。
    abilities.gen_option = {
      type: "",
      id: util.uuid(),
      gen_count: Math.max(1, Math.min(count, 8)),
      generate_all: false,
    };
  } else {
    abilities.blend = {
