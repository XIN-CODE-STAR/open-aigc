    ...
    try {
      ...
      if (typeof image === 'string') {
        if (image.startsWith('data:')) {
          // data URL：对话参考图场景，解码为 Buffer 直传。
          logger.info(`正在处理第 ${i + 1}/${imageCount} 张图片 (data URL)...`);
          const base64Data = image.slice(image.indexOf(',') + 1);
          imageId = (await uploadImageBuffer(Buffer.from(base64Data, 'base64'), refreshToken, regionInfo)).uri;
        } else {
...
export async function generateImages(
  ...
  return await generateImagesInternal(userModel, prompt, { ratio, resolution, sampleStrength, negativePrompt, intelligentRatio, count }, refreshToken);
}
...
async function generateImagesInternal(
  ...
  const metricsExtra = buildMetricsExtra({
    ...
    abilityList: [],
    count,
    omitSceneOptions: true,
  });

  // 使用 payload-builder 构建 draft_content
  // gen_count 控制生成数量（服务端缺省一次生成 4 张）
  const draftContent = buildDraftContent({
    componentId,
...
async function generateJimeng4xMultiImages(
  ...
  // 使用 payload-builder 构建 draft_content（多图分镜：一次生成 4 张）
  const draftContent = buildDraftContent({
    componentId,
    generateType: "generate",
    coreParam,
