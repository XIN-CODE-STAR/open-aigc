...
export async function getCredit(refreshToken: string) {
  ...
  logger.info(`[getCredit] 开始查询积分, token长度=${refreshToken.length}`);
  ...
  try {
    ...
    logger.info(`积分信息: 赠送=${gift_credit}, 购买=${purchase_credit}, VIP=${vip_credit}`);
    return {
    ...
    logger.error(`[getCredit] 查询积分失败: ${e.message}`);
    throw e;
  }
}
  ...
  while (retries <= maxRetries) {
    try {
      ...
      const response = await axios.request({
        ...
        proxy: false, // 禁用系统代理
        httpAgent: proxyAgent || DEFAULT_HTTP_AGENT,
        httpsAgent: proxyAgent || DEFAULT_HTTPS_AGENT,
        ..._.omit(options, "params", "headers", "noDefaultParams"),
      });
      logger.info(`请求完成: ${method.toUpperCase()} ${fullUrl} -> ${response.status}`);
